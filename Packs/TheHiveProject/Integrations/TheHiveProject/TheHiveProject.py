    ...
    def get_cases(self, limit: int = DEFAULT_LIMIT, start_time: int = 0):
        ...
        query = {
            "query": [
                ...
                {
                    "_name": "filter",
                    "_gte": {"_field": "_createdAt", "_value": start_time},
                },
                {"_name": "sort", "_fields": [{"_createdAt": "asc"}]},
                {"_name": "page", "from": 0, "to": limit},
            ]
        }
        res = self._http_request("POST", "v1/query", json_data=query, params={"name": "list-cases"})
...
def test():
    ...
    def search_cases(self, args: dict = None):
        if self.version[0] == "4":
            ...
            res = self._http_request("POST", "case/_search", ok_codes=[200, 201, 404], json_data=args, resp_type="response")
        if res.status_code == 404:
            return None
        else:
            cases = res.json()
    ...
    def update_case(self, case_id: str = None, updates: dict = None):
        res = self._http_request("PATCH", f"case/{case_id}", ok_codes=[200, 201, 404], json_data=updates, resp_type="response")
    ...
    def merge_cases(self, first_case_id: str = None, second_case_id: str = None):
        res = self._http_request(
            "POST", f"case/{first_case_id}/_merge/{second_case_id}", ok_codes=[200, 201, 204, 404], resp_type="response"
        )
        if res.status_code not in [200, 201, 204]:
            return (res.status_code, res.text)
        else:
        ...
        if self.version[0] == "4":
            query = {
                "query": [
                    ...
                    {"_name": "tasks"},
                    {"_name": "sort", "_fields": [{"flag": "desc"}, {"order": "asc"}, {"startDate": "asc"}, {"title": "asc"}]},
                    {"_name": "page", "from": 0, "to": 15, "extraData": ["shareCount"]},
                ]
            }
            res = self._http_request(
                "POST",
            ...
            data = {"id": case_id}
            res = self._http_request("POST", "case/task/_search", data=data, ok_codes=[200, 201, 204, 404], resp_type="response")
            if res.status_code != 200:
                return None
            tasks = list(res.json())
        if tasks:
    ...
    def get_task(self, task_id: str = None):
        if self.version[0] == "4":
            ...
            res = self._http_request("GET", f"case/task/{task_id}", ok_codes=[200, 404], resp_type="response")
        if res.status_code != 200:
            return None
        if res.json():
            task = res.json()
    ...
    def create_task(self, case_id: str = None, data: dict = None):
        res = self._http_request("POST", f"case/{case_id}/task", data=data, ok_codes=[201, 200], resp_type="response")
        ...
        if self.version[0] == "4":
            query = {
                "query": [
                    ...
                    {"_name": "page", "from": 0, "to": 10, "extraData": ["actionCount"]},
                ]
            }
            res = self._http_request(
                "POST", "v1/query", params={"name": "case-task-logs"}, json_data=query, ok_codes=[200, 404], resp_type="response"
            )
        else:
            res = self._http_request("GET", f"case/task/{task_id}/log", ok_codes=[200, 404], resp_type="response")
        if res.status_code != 200:
            return []
        else:
            logs = []
    ...
    def get_attachment_data(self, filename: str = None, fileId: str = None):
        headers = self._headers.update({"name": filename})
        data = b""
        ...
        if self.version[0] == "4":
            query4 = {
                "query": [
                    ...
                    {"_name": "page", "from": 0, "to": 15, "extraData": ["seen", "permissions", "shareCount"]},
                ]
            }
            res = self._http_request("POST", "v1/query", ok_codes=[200], params={"name": "observables"}, json_data=query4)
        else:
            query = {"query": {"_and": [{"_parent": {"_type": "case", "_query": {"_id": case_id}}}, {}]}}
            res = self._http_request("POST", "case/artifact/_search", params={"range": "all"}, ok_codes=[200], json_data=query)
    ...
    def update_observable(self, artifact_id: str = None, data: dict = None):
        res = self._http_request(
            "PATCH", f"case/artifact/{artifact_id}", ok_codes=[200, 204, 404], data=data, resp_type="response"
        )
        return res.json() if res.status_code != 404 else None
...
def output_results(
    title: str, outputs: Any, headers: list, outputs_prefix: str, outputs_key_field: str, human_readable: bool = True
):
    if title and outputs and headers and human_readable:
        md = tableToMarkdown(title, outputs, headers)
    else:
        md = None
    return CommandResults(outputs_prefix=outputs_prefix, outputs_key_field=outputs_key_field, outputs=outputs, readable_output=md)
...
def remove_case_command(client: Client, args: dict):
    ...
    return (
        f"Case ID {case_id} permanently removed successfully"
        if permanent == "true"
...
def create_task_command(client: Client, args: dict):
    ...
    return CommandResults(outputs_prefix="TheHive.Tasks", outputs_key_field="id", outputs=task, readable_output=read)
...
def get_case_tasks_command(client: Client, args: dict):
    ...
    if client.get_case(case_id):
        ...
        if tasks:
            ...
            read = tableToMarkdown(
                f"TheHive Tasks For Case {case_id}:", tasks, ["_id", "title", "_createdAt", "_createdBy", "status", "group"]
            )
        else:
...
def get_task_command(client: Client, args: dict):
    ...
    if task:
        ...
        read = tableToMarkdown(f"TheHive Task {task_id}:", task, ["_id", "title", "_createdAt", "_createdBy", "status", "group"])
    else:
...
def update_task_command(client: Client, args: dict):
    ...
    if task:
        ...
        if type(task) is dict:
            ...
            read = tableToMarkdown(
                f"Updated task with id: {task_id}", updated_task, ["_id", "title", "createdAt", "_createdBy", "status", "group"]
            )
        else:
...
def get_user_command(client: Client, args: dict):
    ...
    if user:
        ...
        if user_date_dt:
            user["createdAt"] = user_date_dt.strftime(DATE_FORMAT)
        read = tableToMarkdown(
            f"TheHive User ID {user_id}:", user, ["_id", "name", "roles", "status", "organisation", "createdAt"]
        )
    else:
...
def create_local_user_command(client: Client, args: dict):
    ...
    if result:
        read = tableToMarkdown(
            f"New User {result.get('id', result.get('_id', None))}:",
            result,
            headers=["id", "login", "name", "roles"] if client.version[0] != "4" else ["_id", "login", "name", "profile"],
        )
    else:
...
def list_observables_command(client: Client, args: dict):
    ...
    if observables:
        read = tableToMarkdown(f"Observables for Case {case_id}:", observables, ["data", "dataType", "message"])
    else:
...
def update_observable_command(client: Client, args: dict):
    ...
    read = (
        tableToMarkdown("Updated Observable:", res, ["id", "data", "dataType", "message"])
        if res
...
def get_modified_remote_data_command(client: Client, args: dict):
    ...
    query = {"query": {"_and": [{"_gt": {"updatedAt": last_timestamp}}]}}
...
def fetch_incidents(client: Client, fetch_closed: bool = False):
    ...
    incidents = filter_incidents_by_duplicates_and_limit(
        incidents_res=incidents, last_run=last_run, fetch_limit=max_fetch_param, id_field="name"
    )
